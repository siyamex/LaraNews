...
@section('content')
<div class="max-w-3xl">
    <h1 class="text-2xl font-bold text-gray-900 dark:text-white mb-6">Edit Page</h1>
    <form action="{{ route('admin.pages.update', $page) }}" method="POST" class="space-y-4">
            ...
            <div class="border border-gray-200 dark:border-gray-700 rounded-lg p-4">
                ...
                <div class="space-y-3">
                    ...
                    <div>
                        ...
                        <textarea name="translations[{{ $locale }}][content]" rows="10"
                                  class="w-full rounded-lg border-gray-300 dark:border-gray-600 dark:bg-gray-700 shadow-sm"
                                  {{ $locale === 'dv' ? 'dir=rtl' : '' }}>{{ old("translations.$locale.content", $trans?->content) }}</textarea>
                    </div>
                </div>
            </div>
            @endforeach
        ...
        <div class="flex gap-3">
            <button type="submit" class="px-6 py-2.5 bg-red-600 hover:bg-red-700 text-white rounded-lg text-sm font-medium">Update Page</button>
