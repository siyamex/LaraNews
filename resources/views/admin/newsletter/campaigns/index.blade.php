    ...
    <table class="w-full text-sm">
        <thead class="bg-gray-50 dark:bg-gray-700 text-xs uppercase text-gray-500">
            <tr>
                ...
                <th class="px-4 py-3 text-center">Sent At</th>
            ...
            <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/50">
                ...
                <td class="px-4 py-3 text-center">
                    <span class="px-2 py-0.5 rounded text-xs font-medium
                        {{ match($campaign->status) { 'sent'=>'bg-green-100 text-green-800', 'sending'=>'bg-blue-100 text-blue-800', 'scheduled'=>'bg-yellow-100 text-yellow-800', default=>'bg-gray-100 text-gray-600' } }}">
                        {{ ucfirst($campaign->status) }}
                    </span>
                </td>
                <td class="px-4 py-3 text-center text-gray-600 dark:text-gray-400">{{ number_format($campaign->recipients_count) }}</td>
                <td class="px-4 py-3 text-center text-gray-600 dark:text-gray-400">{{ number_format($campaign->opens_count) }}</td>
